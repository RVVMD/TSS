...
typedef struct {
    char     label[64];
    SignalType type;
    int      idx;       /* generator idx or bus idx */
    int      id;        /* generator bus id or bus id */
    int      visible;
    double   *data;     /* heap-allocated, PLOT_BUF doubles */
} Signal;
...
static gboolean on_draw_plot(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
    ...
    if (app->plot_len < 2 || nvis == 0) {
        ...
        const char *msg = app->plot_len < 2 ? "No simulation data" : "No signals selected";
    ...
    for (int s = 0; s < app->n_signals; s++) {
        ...
        for (int i = 0; i < app->plot_len; i++) {
            ...
            double v = app->signals[s].data[idx];
            if (start < 0) {
                cairo_move_to(cr, px(t), py(v));
                start = i;
            } else {
                cairo_line_to(cr, px(t), py(v));
            }
        }
        cairo_stroke(cr);
        ci++;
...
static gboolean on_plot_motion(GtkWidget *widget, GdkEventMotion *event, gpointer user_data) {
    ...
    if (app->mouse_down && (int)event->x != (int)app->mouse_x0) {
        app->dragging = 1;
...
static gboolean on_plot_scroll(GtkWidget *widget, GdkEventScroll *event, gpointer user_data) {
    ...
    double t0 = app->view_t0, t1 = app->view_t1;
    double mid = (t0 + t1) / 2;
    double span = (t1 - t0) * 0.15;

    /* find time at cursor position */
...
static void file_load_events(App *app) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dlg)) == GTK_RESPONSE_ACCEPT) {
        ...
        if (app->events) { arrfree(app->events); app->events = NULL; app->n_events = 0; }
        if (events_parse(fn, &app->events, &app->arena) == 0) {
            app->n_events = (int)arrlen(app->events);
            populate_events(app);
        }
        g_free(fn);
    }
...
static void build_network_panel(App *app, GtkWidget **box) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(app->bus_tree),
            i, bus_cols[i], r, "text", i, NULL);
    }
    ...
    for (int i = 0; i < 7; i++) {
        GtkCellRenderer *r = gtk_cell_renderer_text_new();
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(app->branch_tree),
            i, br_cols[i], r, "text", i, NULL);
    }
    ...
    for (int i = 0; i < 5; i++) {
        GtkCellRenderer *r = gtk_cell_renderer_text_new();
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(app->gen_tree),
            i, gen_cols[i], r, "text", i, NULL);
    }
    sw = gtk_scrolled_window_new(NULL, NULL);
    ...
    for (int i = 0; i < 4; i++) {
        GtkCellRenderer *r = gtk_cell_renderer_text_new();
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(app->load_tree),
            i, ld_cols[i], r, "text", i, NULL);
    }
    sw = gtk_scrolled_window_new(NULL, NULL);
...
static void populate_network(App *app) {
    ...
    for (int i = 0; i < s->nbus; i++) {
        ...
        gtk_list_store_set(app->bus_store, &it,
            0, s->bus[i].id, 1, s->bus[i].name, 2, btype(s->bus[i].type),
            3, s->bus[i].base_kv, 4, s->bus[i].vm,
            5, s->bus[i].pd * s->base_mva, 6, s->bus[i].qd * s->base_mva, -1);
    }
    for (int i = 0; i < s->nbranch; i++) {
        GtkTreeIter it;
        gtk_list_store_append(app->branch_store, &it);
        gtk_list_store_set(app->branch_store, &it,
            0, s->bus[s->branch[i].from].id, 1, s->bus[s->branch[i].to].id,
            2, s->branch[i].r, 3, s->branch[i].x, 4, s->branch[i].b,
            5, s->branch[i].tap > 0 ? s->branch[i].tap : 1.0,
            6, s->branch[i].status ? "ON" : "OFF", -1);
    }
    for (int i = 0; i < s->ngen; i++) {
        GtkTreeIter it;
        gtk_list_store_append(app->gen_store, &it);
        gtk_list_store_set(app->gen_store, &it,
            0, s->bus[s->gen[i].bus].id, 1, s->gen[i].pg * s->base_mva,
            2, s->gen[i].qg * s->base_mva, 3, s->gen[i].vsched, 4, s->gen[i].mbase, -1);
    }
    for (int i = 0; i < s->nload; i++) {
        GtkTreeIter it;
        gtk_list_store_append(app->load_store, &it);
        gtk_list_store_set(app->load_store, &it,
            0, s->bus[s->load[i].bus].id, 1, s->load[i].p * s->base_mva,
            2, s->load[i].q * s->base_mva, 3, s->load[i].status ? "ON" : "OFF", -1);
    }
}
...
static void sig_toggled(GtkCellRendererToggle *cell, gchar *path_str, gpointer user_data) {
    ...
    GtkTreeIter iter;
    gtk_tree_model_get_iter(GTK_TREE_MODEL(app->sig_store), &iter, path);
    gboolean active;
...
static void build_signal_tree(App *app) {
    ...
    /* generators: δ, Pe, ω, Vt */
    for (int m = 0; m < s->nmachines; m++) {
        int bid = s->bus[s->gen[s->machine[m].gen_idx].bus].id;

        /* parent row */
        char parent_label[32];
        snprintf(parent_label, sizeof(parent_label), "Generator @ Bus %d", bid);
        GtkTreeIter parent;
        gtk_tree_store_append(app->sig_store, &parent, NULL);
        gtk_tree_store_set(app->sig_store, &parent,
            SIG_COL_CHECK, FALSE, SIG_COL_LABEL, parent_label, SIG_COL_IDX, -1, -1);

        /* δ */
        if (app->n_signals < MAX_SIGNALS) {
            Signal *sig = &app->signals[app->n_signals];
            snprintf(sig->label, sizeof(sig->label), "G%d δ (rad)", bid);
            sig->type = SIG_GEN_DELTA;
            sig->idx = m;
            sig->id = bid;
            sig->visible = 1;  /* default visible */
            GtkTreeIter child;
            gtk_tree_store_append(app->sig_store, &child, &parent);
            gtk_tree_store_set(app->sig_store, &child,
                SIG_COL_CHECK, TRUE, SIG_COL_LABEL, sig->label, SIG_COL_IDX, app->n_signals, -1);
            app->n_signals++;
        }
        /* Pe */
        if (app->n_signals < MAX_SIGNALS) {
            Signal *sig = &app->signals[app->n_signals];
            snprintf(sig->label, sizeof(sig->label), "G%d Pe (pu)", bid);
            sig->type = SIG_GEN_PE;
            sig->idx = m;
            sig->id = bid;
            sig->visible = 1;
        ...
        if (app->n_signals < MAX_SIGNALS) {
            Signal *sig = &app->signals[app->n_signals];
            snprintf(sig->label, sizeof(sig->label), "G%d ω (pu)", bid);
            sig->type = SIG_GEN_OMEGA;
            sig->idx = m;
            sig->id = bid;
            sig->visible = 0;
            GtkTreeIter child;
            gtk_tree_store_append(app->sig_store, &child, &parent);
            gtk_tree_store_set(app->sig_store, &child,
                SIG_COL_CHECK, FALSE, SIG_COL_LABEL, sig->label, SIG_COL_IDX, app->n_signals, -1);
            app->n_signals++;
        }
        /* Vt */
        if (app->n_signals < MAX_SIGNALS) {
            Signal *sig = &app->signals[app->n_signals];
            snprintf(sig->label, sizeof(sig->label), "G%d Vt (pu)", bid);
            sig->type = SIG_GEN_VT;
            sig->idx = m;
            sig->id = bid;
            sig->visible = 0;
    ...
    for (int i = 0; i < s->nbus; i++) {
        int bid = s->bus[i].id;
        char parent_label[32];
        snprintf(parent_label, sizeof(parent_label), "Bus %d (%s)", bid, btype(s->bus[i].type));
        GtkTreeIter parent;
        gtk_tree_store_append(app->sig_store, &parent, NULL);
        gtk_tree_store_set(app->sig_store, &parent,
            SIG_COL_CHECK, FALSE, SIG_COL_LABEL, parent_label, SIG_COL_IDX, -1, -1);
        ...
        if (app->n_signals < MAX_SIGNALS) {
            Signal *sig = &app->signals[app->n_signals];
            snprintf(sig->label, sizeof(sig->label), "B%d Vm (pu)", bid);
            sig->type = SIG_BUS_VM;
            sig->idx = i;
            sig->id = bid;
            sig->visible = 1;
        ...
        if (app->n_signals < MAX_SIGNALS) {
            Signal *sig = &app->signals[app->n_signals];
            snprintf(sig->label, sizeof(sig->label), "B%d angle (deg)", bid);
            sig->type = SIG_BUS_VA;
            sig->idx = i;
            sig->id = bid;
            sig->visible = 0;
...
static void build_event_panel(App *app, GtkWidget **box) {
    ...
    for (int i = 0; i < 6; i++) {
        GtkCellRenderer *r = gtk_cell_renderer_text_new();
        gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(app->event_tree),
            i, ev_cols[i], r, "text", i, NULL);
    }
    GtkWidget *sw = gtk_scrolled_window_new(NULL, NULL);
...
static void populate_events(App *app) {
    ...
    for (int i = 0; i < app->n_events; i++) {
        ...
        gtk_list_store_set(app->event_store, &it,
            0, i, 1, app->events[i].time, 2, ftype_name(app->events[i].type),
            3, app->events[i].bus, 4, app->events[i].fault_r, 5, app->events[i].fault_x, -1);
    }
}
...
static gpointer sim_thread_fn(gpointer data) {
    ...
    int sim_n_events = 0;
    for (int i = 0; i < app->n_events; i++) {
        arrpush(sim_events, app->events[i]);
    }
...
static void app_cleanup(App *app) {
    app->sim_running = 0;
    if (app->sim_thread) {
        g_thread_join(app->sim_thread);
        app->sim_thread = NULL;
    }
    for (int s = 0; s < MAX_SIGNALS; s++)
        free(app->signals[s].data);
    arrfree(app->events);
...
static void build_ui(App *app) {
    ...
    gtk_box_pack_start(GTK_BOX(right_vbox), sim_frame, FALSE, FALSE, 0);

    /* signal tree */
    GtkWidget *sig_frame = gtk_frame_new("Signals");
    GtkWidget *sig_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);

    app->sig_store = gtk_tree_store_new(SIG_N_COLS, G_TYPE_BOOLEAN, G_TYPE_STRING, G_TYPE_INT, G_TYPE_INT);
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(app->sig_tree),
        0, "", r_check, "active", SIG_COL_CHECK, NULL);

    GtkCellRenderer *r_text = gtk_cell_renderer_text_new();
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(app->sig_tree),
        1, "Signal", r_text, "text", SIG_COL_LABEL, NULL);
    ...
    gtk_box_pack_start(GTK_BOX(right_vbox), sig_frame, FALSE, FALSE, 0);
